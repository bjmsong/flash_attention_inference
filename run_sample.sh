...
benchmark_fai_prefill_seq() {
    ...
    b=128
    seq=(1 2 4 8 16 32 64 128 256 512 768 1024 1536 2048 2560 3072)
    hq=32
    hk=32
    d=128
    ic=true
    ...
    for s in ${seq[@]};
    do
        evaluate_fai $b $s $s $hq $hk $d $ic $id $ih $pf $lp
        # ncu_fai $b $s $s $hq $hk $d $ic $id $ih $pf $lp
    done
}

benchmark_fai_prefill_batch() {
    echo "Evaluating Prefill Batch"
    batch=(1 2 4 8 16 32 64 128 256 512 768 1024 1536 2048 2560 3072)
    s=128
    hq=32
    hk=32
    d=128
...
benchmark_fai_decoding_seq() {
    ...
    b=128
    sq=1
    seq_k=(1 2 4 8 16 32 64 128 256 512 768 1024 1536 2048 2560 3072)
    hq=32
    hk=32
    d=128
    ic=false
    ...
    for sk in ${seq_k[@]};
    do
        evaluate_fai $b $sq $sk $hq $hk $d $ic $id $ih $pf $lp
        # ncu_fai $b $s $s $hq $hk $d $ic $id $ih $pf $lp
    done
}
...
# GQA/MQA
# nohup $WORK_PATH/output/bin/flash_attention_inference -b=2 -sq=256 -sk=256 -hq=64 -hk=8 -d=128 -is_causal=true -num_splits=0 -is_alibi=false -is_decoding=false -is_hybrid=false -prefill_fraction=0 -warmup_iterations=1 -profiling_iterations=10 -sleep_duration=100 -enable_check=true > log/fai_2_256_256_64_8_128_0.log 2>&1 &
...
# Hybrid
# nohup $WORK_PATH/output/bin/flash_attention_inference -b=2 -sq=256 -sk=256 -hq=32 -hk=32 -d=128 -is_causal=true -num_splits=0 -is_alibi=false -is_decoding=false -is_hybrid=true -prefill_fraction=50 -warmup_iterations=1 -profiling_iterations=10 -sleep_duration=100 -enable_check=true > log/fai_2_256_256_32_32_128_50.log 2>&1 &
...
# Alibi
# nohup $WORK_PATH/output/bin/flash_attention_inference -b=2 -sq=256 -sk=256 -hq=32 -hk=32 -d=128 -is_causal=true -num_splits=0 -is_alibi=true -is_decoding=false -is_hybrid=false -prefill_fraction=0 -warmup_iterations=1 -profiling_iterations=10 -sleep_duration=100 -enable_check=true > log/fai_2_256_256_32_32_128_0.log 2>&1 &
# sudo ncu --set full --target-processes all --force-overwrite -o ncu/fai_2_256_256_32_32_128_0 $WORK_PATH/output/bin/flash_attention_inference -b=2 -sq=256 -sk=256 -hq=32 -hk=32 -d=128 -is_causal=true -num_splits=0 -is_alibi=true -is_decoding=false -is_hybrid=false -prefill_fraction=0 -warmup_iterations=1 -profiling_iterations=1 -sleep_duration=100 -enable_check=false > log/ncu_fai_2_256_256_32_32_128_0.log 2>&1
